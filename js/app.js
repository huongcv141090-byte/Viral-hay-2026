...
(function () {
    ...
    function initProUI() {
        ...
        /* Bug fix: khoi tao model TokenForge rieng */
        $('pro-model-tf').value = c.models.tfChat || '';
        ...
        /* Bug fix: luu model TokenForge rieng khi doi */
        $('pro-model-tf').addEventListener('change', () => {
            const v = $('pro-model-tf').value.trim();
            if (v) VFPro.updateConfig({ models: { tfChat: v } });
            refreshProState();
        });
        ...
        /* Multi-key pools */
        function keyListToText(arr) { return Array.isArray(arr) ? arr.join('\n') : ''; }
        function refreshPoolHints() {
            const s = VFPro.keyPoolStatus();
            const hi = (id, n, idx) => {
                const el = $(id);
                if (el) el.textContent = n > 1 ? `${n} key trong pool (dang dung key #${idx + 1})` : n === 1 ? '1 key' : '';
            };
            hi('hint-gemini-pool', s.gemini, s.geminiIdx);
            hi('hint-openai-pool', s.openai, s.openaiIdx);
            hi('hint-tf-pool', s.tf, s.tfIdx);
        }
        $('pro-gemini-keys').value = keyListToText(c.geminiKeys);
        $('pro-openai-keys').value = keyListToText(c.openaiKeys);
        $('pro-tf-keys').value = keyListToText(c.tfKeys);
        refreshPoolHints();
        $('pro-gemini-keys').addEventListener('change', () => {
            VFPro.updateConfig({ geminiKeys: $('pro-gemini-keys').value });
            refreshPoolHints(); refreshProState();
        });
        $('pro-openai-keys').addEventListener('change', () => {
            VFPro.updateConfig({ openaiKeys: $('pro-openai-keys').value });
            refreshPoolHints(); refreshProState();
        });
        $('pro-tf-keys').addEventListener('change', () => {
            VFPro.updateConfig({ tfKeys: $('pro-tf-keys').value });
            refreshPoolHints(); refreshProState();
        });
